...
BST *bst_create(i64 *arr, siz n) {
	...
	root->value = arr[0];
	for(siz i = 1; i < n; i++) bst_insert(root, arr[i]);
	return root;
}
...
i64 bst_count_internal(BST *bst) {
	if(bst == NULL)
		return 0;
	if(bst->left || bst->right)
		return bst_count_internal(bst->right) + bst_count_internal(bst->left) +
		       1;
	return 0;
}
...
u8 bst_delete(BST **root, i64 val) {
	...
	switch(delete_type) {
		...
		case 2: {
			BST *child = ptr->left == NULL ? ptr->right : ptr->left;
			if(prev == NULL)
				*root = child;
...
u8 bst_search(BST *bst, i64 value) {
	for(BST *tmp = bst; tmp != NULL;
	    tmp      = value > tmp->value ? tmp->right : tmp->left)
        if(tmp->value == value)
            return 1;
	return 0;
}
...
#define bst_order_test(ordername, output_order)                              \
	static i64 test_bst_##ordername() {                                      \
		bst_test_pointer = 0;                                                \
		arr_fill_rand(bst_test_array, config.bst_test_item_count, 3289983,   \
		              SAMPLE_CASE_AVERAGE);                                  \
		bst_##ordername(bst_order_test_tree, test_bst_order_process);        \
		i64 ret =                                                            \
		    check_sort(bst_test_array, bst_count_nodes(bst_order_test_tree), \
		               SORT_TYPE_##output_order);                            \
		return ret;                                                          \
	}

bst_order_test(inorder, ASCENDING) bst_order_test(inorder_nonrec, ASCENDING)
...
#define bst_count_test(name, value)                                            \
	static i64 test_bst_count_##name() {                                       \
		if(bst_order_test_tree == NULL) {                                      \
			bst_test_pointer = 0;                                              \
			bst_test_array   = arr_new(config.bst_test_item_count);            \
			tst_pause("Creating binary tree");                                 \
			arr_fill_rand(bst_test_array, config.bst_test_item_count, 20003,   \
			              SAMPLE_CASE_AVERAGE);                                \
			i64 *bst_input_array = arr_new(config.bst_test_item_count);        \
			arr_fill_rand(bst_input_array, config.bst_test_item_count, 898213, \
			              SAMPLE_CASE_BEST);                                   \
			bst_order_test_tree =                                              \
			    bst_create(bst_input_array, config.bst_test_item_count);       \
		...
		return ret;                                                            \
                    ...
                    bst_count_test(nodes, config.bst_test_item_count)
                        bst_count_test(internal, config.bst_test_item_count - 1)
...
#undef bst_count_test
	...
	if(bst_find_height(bst_order_test_tree) != config.bst_test_item_count - 1)
		return 0;
	return 1;
}
...
void test_bst() {
	...
	TEST("Creation", test_bst_create(config.bst_test_item_count));
	BST *bst = bst_test_gen;
	TEST("Searching", bst_search(bst, bst->value));
	TEST("Insertion",
	     test_bst_insert(bst, random_at_most(config.bst_test_item_count)));
