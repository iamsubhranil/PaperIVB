...
static u8 test_regula_falsi() {
	double res =
	    numerical_regula_falsi(root_test_fx1, 0, 2.5, 0.00000000001, 100);
	if(dne(res, 2))
		return 0;
...
static u8 test_newton_raphson() {
	double res = numerical_newton_raphson(root_test_fx1, root_test_fx1d, 0,
	                                      0.00000000001, 100);
	if(dne(res, 2))
		return 0;
	res =
	    numerical_newton_raphson(root_test_fx2, root_test_fx2d, 3, 0.001, 100);
	if(fabs(res - 3.2968) > 0.001)
		return 0;
	return 1;
}
...
static double **create_matrix_2d(siz n, siz m, ...) {
	...
	for(siz i = 0; i < n; i++) mat[i] = (double *)malloc(sizeof(double) * m);
...
static void free_matrix_2d(double **mat, siz n) {
	for(siz i = 0; i < n; i++) free(mat[i]);
	free(mat);
}

static u8 test_gauss_elimination() {
	double **coeff    = create_matrix_2d(3, 4, 20.0, 15.0, 12.0, 6.0, 6.0, 4.0,
                                      3.0, 0.0, 6.0, 3.0, 2.0, 6.0);
