...
typedef enum { STATUS_OK = 0, STATUS_FULL = 1, STATUS_EMPTY = 2 } QueueStatus;
...
#define check_if_full(queue) queue->status = queue->front == queue->rear;

#define decr_pointer(queue, val) --(*val) < 0 ? *val = queue->size - 1 : 1;

#define incr_pointer(queue, val) ++(*val) == queue->size ? *val = 0 : 1;
...
static i64 test_queue_count(CircularQueue *queue) {
	...
	if(queue_count(queue) != testSize)
		return 0;
	while(!queue_is_full(queue)) queue_insert(queue, random_at_most(988945));
	if(queue_count(queue) != queue->size)
		return 0;
	while(!queue_is_empty(queue)) queue_delete(queue);
...
static i64 test_queue_circularity(CircularQueue *queue) {
	while(!queue_is_full(queue)) queue_insert_at_front(queue, 34);
	i64 last;
	while(!queue_is_empty(queue)) queue_delete_from_rear(queue);
	while(!queue_is_full(queue)) queue_insert(queue, 34);
	while(!queue_is_empty(queue)) last = queue_delete(queue);
	return last == 34;
}

void test_queue() {
	tst_suite_start("Queue", 7);
	TEST("Creation", test_queue_new(config.queue_test_item_count));
	TEST("Insertion",
	     test_queue_insert(testQueue, config.queue_test_item_count));
	TEST("Insertion At Front",
	     test_queue_insert_at_front(testQueue, config.queue_test_item_count));
	...
	TEST("Circularity", test_queue_circularity(testQueue));
