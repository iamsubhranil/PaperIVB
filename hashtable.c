...
typedef enum { TABLE_OPEN, TABLE_CLOSED } TableType;
...
static siz hp_quadratic(HT *table, siz gen_index) {
	...
	while(table->used[gen_index]) {
		gen_index = (orig + (i * i));
		while(gen_index >= table->table_size) gen_index -= table->table_size;
...
static siz hp_random(HT *table, siz gen_index) {
	...
	while((m > h ? m / h : h / m) > 1) h = random_at_most(table->table_size);
	siz idx = orig;
	do {
		idx = ((idx + m) % h) + 1;
		if(idx == orig)
...
HT *hashtable_create_open(siz size, HashFunction fun) {
	...
	switch(fun) {
		case HF_DIVISION: ht->func = hf_division; break;
		case HF_FOLDING: ht->func = hf_folding; break;
		default: ht->func = hf_midsq;
	}
...
HT *hashtable_create_closed(siz size, HashFunction fun,
                            ProbingTechnique prober) {
	HT *ht         = (HT *)malloc(sizeof(HT));
	ht->table_size = size;
	...
	switch(prober) {
		case PROBE_LINEAR: ht->prober = hp_linear; break;
		case PROBE_RANDOM: ht->prober = hp_random; break;
		case PROBE_QUADRATIC: ht->prober = hp_quadratic; break;
	}
...
static u8 test_hashtable_create_closed() {
	test_closed_ht = hashtable_create_closed(config.hashtable_test_item_count,
	                                         HF_MIDSQUARE, PROBE_LINEAR);
	u8 ret         = test_closed_ht ? 1 : 0;
	if(test_closed_ht->table_size != (siz)config.hashtable_test_item_count)
		ret &= 0;
...
static u8 test_hashtable_search(HT *ht) {
	...
	if(hashtable_search(ht, random_at_most(rand())))
		return 0;
	i64 random = random_at_most(config.hashtable_test_item_count);
...
static u8 test_hashtable_insert_closed(HT *ht) {
	...
	arr_fill_rand(arr, config.hashtable_test_item_count,
	              config.hashtable_test_item_count, SAMPLE_CASE_BEST);
	for(i64 i = 0; i < config.hashtable_test_item_count; i++) {
