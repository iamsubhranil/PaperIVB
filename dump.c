...
void dump_load(const char *name) {
	...
	for(siz i = 0; i < size; i++) fscanf(f, "%" Si64, &dataset[i]);
	fclose(f);
...
void dump_data(const char *name) {
	...
	fprintf(f, "%" Psiz, size);
	for(siz i = 0; i < size; i++) fprintf(f, " %" Pi64, dataset[i]);
	fclose(f);
...
static ProgramInfo get_program_info(const char *address) {
	...
	siz   idx    = 0;
	for(siz tmp = bak; tmp < i; tmp++, idx++) offset[idx] = address[tmp];
	offset[idx] = '\0';
	p.offset    = offset;
	return p;
}
...
static int addr2line(const char *addr) {
...
#else
	sprintf(addr2line_cmd, "addr2line -f -i -p -e %s %s", info.name,
	        info.offset);
#endif
	...
	printf(ANSI_COLOR_RED ANSI_FONT_BOLD "[#%" Pi64 "] " ANSI_COLOR_RESET,
	       stack_count--);
	fflush(stdout);
...
static void posix_signal_handler(int sig, siginfo_t *siginfo, void *context) {
	...
	printf("\n");
	switch(sig) {
		case SIGSEGV: err("Caught SIGSEGV: Segmentation Fault!"); break;
		case SIGINT:
			err("Caught SIGINT: Interactive attention signal (usually "
			    "ctrl+c)!");
			break;
		case SIGFPE:
			switch(siginfo->si_code) {
				case FPE_INTDIV:
					...
					break;
				case FPE_INTOVF: err("Caught SIGFPE: Integer overflow!"); break;
				case FPE_FLTDIV:
				...
				case FPE_FLTSUB:
					...
					break;
				default: err("Caught SIGFPE: Arithmetic Exception!"); break;
			}
		case SIGILL:
			switch(siginfo->si_code) {
				case ILL_ILLOPC: err("Caught SIGILL: Illegal opcode!"); break;
				case ILL_ILLOPN: err("Caught SIGILL: Illegal operand!"); break;
				case ILL_ILLADR:
					err("Caught SIGILL: Illegal addressing mode!");
					break;
				case ILL_ILLTRP: err("Caught SIGILL: Illegal trap!"); break;
				case ILL_PRVOPC:
				...
				case ILL_BADSTK:
					...
					break;
				default: err("Caught SIGILL: Illegal Instruction!"); break;
			}
			break;
		case SIGTERM:
			err("Caught SIGTERM: A termination request was sent to the "
			    "program!");
			break;
		case SIGABRT:
...
static void    set_signal_handler() {
    /* setup alternate stack */
    {
        ...
        if(sigaltstack(&ss, NULL) != 0) {
            err("Failed to set alternate sigaltstack!");
        }
    }
...
#define REG_HAND(x)                          \
	if(sigaction(x, &sig_action, NULL) != 0) \
	err("Unable to register handler for " #x "!")
